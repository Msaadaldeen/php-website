...
class FormValidation implements ValidationInterface
{
    ...
    {
        $wordSeparator = separator($field);
        if (!isset($this->formInput[$field]) || empty($this->formInput[$field])) {
            return "The {$wordSeparator} field is required.";
        }
    ...
    private function available(string $field, string $satisfier)
    {
        ...
        $this->db->query("SELECT 1 FROM {$satisfier} WHERE {$field} = :placeholder", [':placeholder' => $value]);
